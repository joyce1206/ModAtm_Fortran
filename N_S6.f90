...
!WRITE(1,*)'set size0 square; set xrange[0:1]; set yrange[0:1]; unset key'
...
!Escritura de las componentes de las velocidad a los largo de las líneas centrales de la cavidad
OPEN(1,FILE='u_y'//itchar(1:len_trim(itchar))//'.txt',STATUS='REPLACE')
i=INT(nx/2)+1
DO j=0, ny+1
 WRITE(1,*)yc(j),uc(i,j)
END DO
...
OPEN(1,FILE='v_x'//itchar(1:len_trim(itchar))//'.txt',STATUS='REPLACE')
j=INT(ny/2)+1
DO i=0, nx+1
 WRITE(1,*)xc(i),vc(i,j)
END DO
